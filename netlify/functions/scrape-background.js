...
async function saveStored(token, data) {
  ...
  try {
    var storeId = await getOrCreateStore(token);
    if (!storeId) return;
    await timeoutFetch("https://api.apify.com/v2/key-value-stores/" + storeId + "/records/" + RECORD_KEY + "?token=" + token, {
      method: "PUT",
...
async function scrapeYouTube(ytKey) {
  ...
  try {
    var handleRes = await timeoutFetch("https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&forHandle=" + YOUTUBE_CHANNEL_HANDLE + "&key=" + ytKey, {}, 10000);
    ...
    var channelDetailRes = await timeoutFetch("https://www.googleapis.com/youtube/v3/channels?part=contentDetails&id=" + channelId + "&key=" + ytKey, {}, 10000);
    ...
    if (uploadsPlaylistId) {
      ...
      for (var page = 0; page < 10; page++) {
        ...
        var plRes = await timeoutFetch(plUrl, {}, 10000);
    ...
    for (var batch = 0; batch < allVideoIds.length; batch += 50) {
      ...
      var statsRes = await timeoutFetch("https://www.googleapis.com/youtube/v3/videos?part=statistics,snippet,contentDetails&id=" + batchIds + "&key=" + ytKey, {}, 10000);
    ...
    // Get ALL comments from ALL videos — no limits
    var allComments = [];
    for (var i = 0; i < videoDetails.length; i++) {
      try {
        var nextPageToken = "";
        for (var cp = 0; cp < 50; cp++) {
          ...
          var cr = await timeoutFetch(commUrl, {}, 10000);
...
async function scrapeInstagramPosts(token, username, platformLabel, episodes) {
  ...
  try {
    var res = await timeoutFetch("https://api.apify.com/v2/acts/apify~instagram-post-scraper/run-sync-get-dataset-items?token=" + token + "&timeout=60", {
      method: "POST", headers: { "Content-Type": "application/json" },
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      posts.push({
        ...
        likes: p.likesCount || p.likes || 0, commentCount: p.commentsCount || p.comments || 0,
...
async function scrapeTikTok(token, username, platformKey, episodes) {
  if (!token) return { posts: [], followers: 0 };
  try {
    var res = await timeoutFetch("https://api.apify.com/v2/acts/clockworks~tiktok-scraper/run-sync-get-dataset-items?token=" + token + "&timeout=30", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ profiles: ["https://www.tiktok.com/@" + username], resultsPerPage: 30, shouldDownloadVideos: false })
    }, 35000);
    if (!res.ok) return { posts: [], followers: 0 };
    var items = await res.json();
...
exports.handler = async (event) => {
  ...
  try {
    ...
    var settled = await Promise.allSettled([
      scrapeInstagramPosts(apifyToken, IG_ACCOUNT, "instagram", episodes),
      scrapeInstagramPosts(apifyToken, IG_HOSTS_ACCOUNT, "instagram_hosts", episodes),
    ...
    var igPostsResult = settled[0].status === "fulfilled" ? settled[0].value : { posts: [], followers: 0 };
    var igHostsPostsResult = settled[1].status === "fulfilled" ? settled[1].value : { posts: [], followers: 0 };
    var igFollowers = settled[2].status === "fulfilled" ? settled[2].value : 0;
    var igHostsFollowers = settled[3].status === "fulfilled" ? settled[3].value : 0;
    var ttResult = settled[4].status === "fulfilled" ? settled[4].value : { posts: [], followers: 0 };
    var ttHostsResult = settled[5].status === "fulfilled" ? settled[5].value : { posts: [], followers: 0 };
    ...
    function retagPosts(posts) {
      return posts.map(function(p) { p.episode = detectEpisode(p.title, p.date, episodes); return p; });
    }
    ...
    var result = {
      ...
      debug: { yt: ytResult.posts.length, ig: igPostsResult.posts.length, igH: igHostsPostsResult.posts.length, tt: ttResult.posts.length, ttH: ttHostsResult.posts.length, comments: allComments.length }
    };
